...
func _ready():
	generate_terrain(Vector2(0, 0), 0)  # Start with the initial chunk

# Generate terrain based on chunk coordinates
func generate_terrain(chunk_coords: Vector2, base_height: float):
	...
	for z in range(zSize + 1):
		for x in range(xSize + 1):
			...
			var y = base_height  # Start at the base height
