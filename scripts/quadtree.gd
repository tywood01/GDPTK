...
var player_position_last: Vector3 = Vector3.ZERO
@export var player: Node3D  # Reference to the player node
# Quadtree specific properties
@export var quadtree_size: float = 512  # Size of the world
@export var max_chunk_depth: int = 8    # Maximum subdivision depth
...
func _process(delta):
	if player:
		...
		if player_position != player_position_last:
			if floor(player_position) != player_position_last:
				update_chunks()
				player_position_last = floor(player_position)
...
func update_chunks():
	...
	if player:
		quadtree.subdivide(player.global_transform.origin)
